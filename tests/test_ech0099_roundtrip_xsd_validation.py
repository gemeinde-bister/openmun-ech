...
def check_namespaces(elem: ET.Element) -> list[str]:
    ...
    # Check baseData specifically (known potential bug location)
    for base_data in elem.iter():
        if 'baseData' in base_data.tag:
            # baseData should be in eCH-0099 namespace, not eCH-0011
            if 'eCH-0011' in base_data.tag:
                issues.append(
                    f"baseData uses wrong namespace: {base_data.tag}\n"
                    f"  Expected: {{http://www.ech.ch/xmlns/eCH-0099/2}}baseData\n"
                    f"  Got:      {base_data.tag}"
                )
...
class TestECH0099RoundtripXSDValidation:
    """Test eCH-0099 v2.1 XSD validation with auto-downloaded schemas."""

    def test_schemas_available(self):
        """Verify all required eCH schemas can be downloaded."""
        schema_path = ensure_schemas()
        assert schema_path.exists(), f"eCH-0099 schema not found at {schema_path}"

    def test_production_file_validates_against_xsd(self, ech0099_files):
        """Test that original production files validate against XSD.
        ...
        try:
            ...
            pytest.fail(
                f"Original production file does NOT validate against eCH-0099 XSD!\n"
                f"File: {xml_file}\n"
                f"Error: {e}\n"
    ...
    def test_roundtrip_output_validates_against_xsd(self, ech0099_files):
        ...
        temp_file = Path('/tmp/test_ech0099_exported.xml')
        ...
        try:
            ...
            pytest.fail(
                f"Exported XML does NOT validate against eCH-0099 XSD!\n"
    ...
    def test_all_production_files_validate(self, ech0099_files):
        """Test all production files validate after roundtrip."""
        if not ech0099_files:
            pytest.skip("No eCH-0099 files available")
        ...
        for xml_file in ech0099_files:
            try:
                ...
                # Write to temp file
                temp_file = Path(f'/tmp/test_ech0099_{xml_file.stem}.xml')
                exported_tree = ET.ElementTree(exported)
                ET.indent(exported_tree, space='  ')
                exported_tree.write(temp_file, encoding='utf-8', xml_declaration=True)
        ...
        if failures:
            msg = f"\n{len(failures)}/{len(ech0099_files)} files failed XSD validation:\n\n"
